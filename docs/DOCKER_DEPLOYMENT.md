...
## 🔄 Пересборка проекта после изменений в коде

После внесения изменений в код (backend, frontend, shared пакет) необходимо пересобрать контейнеры, чтобы изменения вступили в силу.

### Вариант 1: Автоматическая пересборка (рекомендуется)

В проекте есть готовый PowerShell скрипт для пересборки:
...
2. **Запустите скрипт пересборки:**
   ...
   **Что делает скрипт:**
   - Проверяет статус Docker
   - Останавливает контейнеры (сохраняя данные БД)
   - Пересобирает образы с учётом изменений в коде
...
3. **Дополнительные опции:**
   ...
   **Пересборка только конкретного сервиса (быстрее):**
   ```powershell
   # Пересобрать только backend
   .\rebuild-docker.ps1 -Service api
   
   # Пересобрать только frontend
   .\rebuild-docker.ps1 -Service web
   ```

   **Полная пересборка без кеша (если есть проблемы):**
   ```powershell
   .\rebuild-docker.ps1 -NoCache
   ```
   ⚠️ Займёт больше времени (5-10 минут), но гарантирует чистую сборку.

   **Пересборка конкретного сервиса без кеша:**
   ```powershell
   .\rebuild-docker.ps1 -Service api -NoCache
   ```
...
3. **Пересобрать только конкретный сервис:**
   ```powershell
   # Только backend
...
Пересборка необходима после изменений в:
- ✅ Коде backend (`apps/api/src/`)
- ✅ Коде frontend (`apps/web/src/`)
- ✅ Shared пакете (`packages/shared/`)
- ✅ Dockerfile'ах
- ✅ Зависимостях (`package.json`, `package-lock.json`)
- ✅ Prisma схеме (`apps/api/prisma/schema.prisma`)

**Не требуется пересборка** для изменений в:
- ❌ `.env` файле (достаточно перезапуска: `docker-compose restart`)
- ❌ `docker-compose.yml` (только конфигурация, не код)
