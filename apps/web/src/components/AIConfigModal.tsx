                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {modelsByProvider[provider.id]?.length > 0 && (
                      <span className="text-xs px-2 py-0.5 bg-blue-600/20 text-blue-400 rounded-full border border-blue-600/30">
                        {modelsByProvider[provider.id].length} {modelsByProvider[provider.id].length === 1 ? 'модель' : 'моделей'}
                      </span>
                    )}
                  </div>
                </div>
  ...
  const renderModelsTab = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Заголовок с кнопкой обновления и счетчиком моделей */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {totalModels > 0 && (
              <span className="text-sm px-3 py-1 bg-purple-600/20 text-purple-400 rounded-full border border-purple-600/30 font-medium">
                {totalModels} {totalModels === 1 ? 'модель доступна' : totalModels < 5 ? 'модели доступно' : 'моделей доступно'}
              </span>
            )}
          </div>
          <button
            onClick={() => {
              ...
              const activeProviders = PROVIDERS.filter(p => getKeyStatus(p.id) === 'active');
              activeProviders.forEach(p => refreshModelsMutation.mutate(p.id));
            }}
            disabled={refreshModelsMutation.isPending || modelsLoading}
          ...
          >
            <svg 
              className={`w-4 h-4 ${refreshModelsMutation.isPending ? 'animate-spin' : ''}`} 
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
            ...
            {refreshModelsMutation.isPending ? 'Обновление...' : 'Обновить модели'}
          </button>
        </div>
