// src/components/ui/SplitPreviewList.js
import React from 'react';
import { useTranslation } from 'react-i18next';

const SplitPreviewList = ({ parts = [], selected = [], onTogglePart, onToggleAll }) => {
  ...
  return (
    <div className="max-h-96 overflow-y-auto border border-gray-700 rounded">
      <div className="sticky top-0 bg-gray-800 border-b border-gray-700 p-2 flex justify-between items-center">
        ...
        <div>
          <button
            className="text-xs px-2 py-1 bg-blue-800 text-blue-200 rounded hover:bg-blue-700 mr-2"
      ...
      <div className="divide-y divide-gray-700">
        {parts.map((part, index) => (
          <div 
            key={index} 
            className={`p-3 hover:bg-gray-750 ${selected[index] ? 'bg-blue-900 bg-opacity-20' : ''}`}
          >
            <div className="flex items-start">
              ...
              <div className="flex-grow">
                <div className="text-sm font-mono whitespace-pre-wrap overflow-hidden text-gray-300">
                  {getPreview(part)}
                </div>
      ...
      {parts.length === 0 && (
        <div className="p-4 text-center text-gray-500">
          {t('splitModal.previewList.noItems')}
        </div>
      )}
    </div>
  );
